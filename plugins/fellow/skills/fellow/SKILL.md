...
- **Reports missing config** → run [Onboarding](#onboarding). Missing config is the expected first-run state, not a failure — don't report it to the user as an error.
...
Only when `config check` reports missing configuration. The goal is a working setup, so finish by verifying — not by writing a file and hoping.
