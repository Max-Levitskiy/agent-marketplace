...
export function resolveCredential(ref: CredentialRef): string {
  let value: string;
  ...
  switch (ref.source) {
    ...
    case "dotenv": {
      ...
      try {
        ...
        throw new Error(`Cannot read ${p} (referenced by credentials.apiKey.path).`);
      }
      const line = text
        .split("\n")
  ...
  if (!value) throw new Error("Credential resolved to an empty value.");
  return value;
}
...
export function describeCredential(ref: CredentialRef): string {
  switch (ref.source) {
    case "1password": return `1Password: ${ref.ref}${ref.account ? ` (account ${ref.account})` : ""}`;
    case "env": return `environment variable ${ref.var}`;
    case "dotenv": return `${ref.path} → ${ref.var}`;
    case "keychain": return `macOS Keychain: ${ref.service} / ${ref.account}`;
    case "command": return `command: ${ref.command}`;
